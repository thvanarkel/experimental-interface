...
client = mqtt.Client("wii-board")
client.on_connect = on_connect
client.on_message = on_message

broker_address="broker.shiftr.io"
client.username_pw_set("wii-board", "72140d05bbe63667")
...
class EventProcessor:
    ...
    def update_values(self, event):
        client.publish("topLeft", event.topLeft)
        client.publish("topRight", event.topRight)
        client.publish("bottomLeft", event.bottomLeft)
        client.publish("bottomRight", event.bottomRight)
        print "sending..."
	#def mass(self, event):
    ...
    def receive(self):
        while self.status == "Connected" and not self.processor.done:
            ...
            if intype == INPUT_STATUS:
                ...
                self.processor.update_values(self.createBoardEvent(data[2:12]))
            else:
